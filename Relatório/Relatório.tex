...
Usando a Equação ref{eq:pendulo-tf-sem-cond-inv}, calcula-se os valores de sensibilidade($S$), resolução de entrada($RE$), resolução de saída($RS$) e erro de linearidade($EL$).

\begin{equation}
	S = dfrac{\partial V(\theta)}{\partial \theta} = 0.028
	\label{eq:sensibilidade-sem-cond}
\end{equation}

\begin{equation}
	RE = dfrac{\partial V(\theta)}{\partial \theta} = 0.028
	\label{eq:resolucao-sem-cond}
\end{equation}
